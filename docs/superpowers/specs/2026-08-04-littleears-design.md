...
> 本文件同时用作项目开发进度跟踪。每完成一个里程碑，更新此表。

| 里程碑 | 状态 | 完成日期 | 备注 |
|--------|------|----------|------|
| **M1 — Hello Ears** | ✅ 完成 | 2026-08-04 | 双播放模式、文件树、路径安全 |
| **M2 — 完整播放体验** | ✅ 完成 | 2026-08-04 | 播放管理器、循环模式、目录播放 |
| **M3 — 远程 + 体验** | ⬜ 未开始 | — | 密码认证、花生壳、移动端 |
| **M4 — 锦上添花** | ⬜ 未开始 | — | 定时停止、历史、收藏 |

### M1 验收记录（2026-08-04）

**功能验证：**
- ✅ `/api/files` 返回文件树 JSON（中文路径、多级目录正常）
- ✅ 浏览器模式播放：`<audio>` 流式播放真实音频（Brown Bear 154.7s 完整加载）
- ✅ 音箱模式播放：mpg123 子进程播放（真实音频），自然结束触发回调
- ✅ 播放工具缺失时自动回退 ffplay
- ✅ 路径穿越防护（`../../etc/passwd` 被拒绝）
- ✅ 模式切换自动停止旧播放（音箱/浏览器双向，含孤儿进程清理）
- ✅ Range 请求支持（HTTP 206，进度条拖动基础）
- ✅ 页面渲染：文件树可折叠、模式切换、状态栏

**内容管理：**
- ✅ 廖彩杏 1阶段 62 个 mp3 拷贝并重排（存放于 media/廖彩杏/1阶段/，预留 2阶段）
- ✅ 重排规则：`{序号}-{子序号}_{书名}.mp3`（1-a、2-b…），全部重新编号 1~20
- ✅ 自然排序修复：`1-a → 1-b → 2-a → ... → 20-a`（数值段按大小而非字典序）

**环境：**
- 开发机 Ubuntu 22.04，安装 mpg123 1.29.3 + ffmpeg
- 与树莓派（Raspberry Pi OS, Debian 系）工具链完全兼容

### M2 验收记录（2026-08-04）

**功能验证：**
- ✅ player-manager 播放调度核心（状态机 IDLE/PLAYING/PAUSED）
- ✅ 三种循环模式：单曲(重播)/顺序(下一首)/随机(不重复)，播完即停
- ✅ 目录级播放：递归收集音频（廖彩杏/1阶段 62 首队列）
- ✅ 暂停/恢复：SIGSTOP/SIGCONT 挂起进程（进度保留）
- ✅ 停止：清空队列回 idle
- ✅ 下一首：代际号防连锁快进（SIGTERM exit 竞态修复）
- ✅ 浏览器模式自动下一首：<audio> ended → /api/next
- ✅ UI 控制栏：进度条、计数、暂停/停止/上一首/下一首
- ✅ API 契约统一：play/next/pause 均返回完整 status

**修复的 Bug：**
- 快速连点导致多 mpg123 进程（代际号 + killCurrent 分离）
- next 触发连锁快进（exit 回调代际校验）
- GET 请求误发 POST body（API 辅助函数显式 method）

### M2 人工测试确认（2026-08-04）

用户在真实设备上人工测试通过后确认，M2 验收记录补充：

- ✅ 音箱模式播放（mpg123 出声）
- ✅ 浏览器模式播放（<audio> 出声 + 自动下一首）
- ✅ 目录播放 + 单文件播放（队列=同目录，不跨目录）
- ✅ 三种循环模式：单曲 🔂 / 顺序 🔁 / 随机 🔀
- ✅ 下一首：同目录递进，队尾循环，进度框不消失
- ✅ 暂停/恢复/停止/进度条
- ✅ 模式切换自动停止旧播放
- ✅ 人工测试通过 → 打 tag v0.2.0
