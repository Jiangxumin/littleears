...
| 音箱播放 | **mpg123**（MP3）/ **ffplay**（MP4） | 轻量稳定，通过 child_process 调用；声卡参数按平台选择：树莓派 arm 直连 `hw:CARD=Headphones`，x86 用系统默认声卡 |
...
| `POST` | `/api/play` | 播放 `{ path, mode, playMode }`，path 可为文件或目录（目录 → 递归收集全部音频为队列），mode: server/browser | M1 |
| `POST` | `/api/next` | 下一首（手动切歌 / 浏览器模式 ended 自动调用） | M2 |
...
│  ──────────────────────────────          │
│  🎵 播放列表（62）   ← 最深层即播放列表    │
│  🎵 1-a_Ape in a cape.mp3   ♪ 正在播放   │
│  🎵 1-b_Ape in a cape.mp3                │
│  🎵 2-a_Brown bear.mp3                   │
│  ...                                    │
└─────────────────────────────────────────┘
```
...
| 3 | 实现暂停/停止/进度条 | 播放控制 |
| 4 | 实现进入/返回式文件导航 | 点击目录进入下级，`← 返回` 回上层，面包屑跳级；目录行 `▶` 播放全部；最深层目录的音频列表即播放列表 |
